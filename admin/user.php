...
<body>
        ...
        <div class="row">
            <div class="col-md-11">
                <div class="table-resposive">
                    <div class = "button">
                        <button class="add-btn" onclick="window.location.href='add-user.php'">Add User</button>
                        ...
                        <form class="form-horizontal row-fluid" action="user.php" method="post">
                            <div class="control-group">
                                <div class="controls">
                                    <input type="text" id="title" name="title" placeholder="Enter Name/ID number of user" class="span8" required>
                        ...
                        if(isset($_POST['submit']))
                            {$s=$_POST['title'];
                                $sql="SELECT * FROM ecclms.user WHERE (id_number='$s' or name like '%$s%') and id_number<>'ADMIN'";
                            }
                        else
                            $sql="SELECT * FROM ecclms.user WHERE id_number<>'ADMIN' LIMIT $offset, $total_rows_per_page";
                        ...
                        <tbody>
                            ...
                            <tr>    
                                ...
                                <td> <?php echo '<img src="data:image/jpeg;base64,'.base64_encode( $row['id_pic'] ).'" width="100px" />';?> </td>
                                <td>
                                    <center>
                                        <a href="admin_viewdetails.php?id=<?php echo $row['id_number']; ?>" class="btn btn-warning">Details</a>
                                        <a href="admin_viewdetails.php?id=<?php echo $row['id_number']; ?>" class="btn btn-danger">Remove</a>
                                    </center>
                                </td>
                            </tr>
                            <?php
    ...
    <script>
        function Export (){
            var conf = confirm("You want to export list of students in to excel file?");
            if(conf == true){
                window.open("export_students.php", '_blank');
            }
        }
    </script>
</body>
