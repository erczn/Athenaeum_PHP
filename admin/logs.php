<?php
    ...
    if($_SESSION['id']){
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) as total_records FROM logins");
        $records = mysqli_fetch_array($result_count);
        $total_records = $records['total_records'];
        $total_no_per_page = ceil($total_records/$total_rows_per_page);

        $sql = mysqli_query($conn, "SELECT logins.id_number, user.name, logins.time_in, logins.time_out, logins.log_date
            FROM logins, user WHERE logins.id_number = user.id_number
            ORDER BY log_date DESC, time_in DESC
            LIMIT $offset , $total_rows_per_page");
        $count = 1;
?>
...
<body>
                    ...
                    <div class="table-resposive">
                        <div class = "button">
                            ...
                            if(isset($_POST['submit']))
                                {$s=$_POST['title'];
                                    $sql="SELECT * FROM ecclms.logins, ecclms.user WHERE user.id_number = '$s' like '%$s%' AND id_number <> 'ADMIN'";
                                }
                            else{
                                $sql="SELECT * FROM ecclms.logins, ecclms.user WHERE user.id_number <> 'ADMIN' LIMIT $offset, $total_rows_per_page";
                            }    
    ...
    <script>
        function Export (){
            var conf = confirm("You want to export attendance in to excel file?");
            if(conf == true){
                window.open("export_attendance.php", '_blank');
            }
        }
    </script>
</body>
